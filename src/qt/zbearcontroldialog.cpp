...
#include "zbearcontroldialog.h"
#include "ui_zbearcontroldialog.h"
...
std::list<std::string> ZBEARControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBEARControlDialog::listMints;

ZBEARControlDialog::ZBEARControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBEARControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBEARControlDialog::~ZBEARControlDialog()
{
    delete ui;
}

void ZBEARControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBEARControlDialog::updateList()
{
...
void ZBEARControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBEARControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBEAR_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));
}

std::vector<CZerocoinMint> ZBEARControlDialog::GetSelectedMints()
{
...
void ZBEARControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
