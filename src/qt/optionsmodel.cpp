...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BEAR);
    ...
    if (!settings.contains("nAnonymizeBEARAmount"))
        settings.setValue("nAnonymizeBEARAmount", 1000);

    nAnonymizeBEARAmount = settings.value("nAnonymizeBEARAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBEARAmount"))
        SoftSetArg("-anonymizebearamount", settings.value("nAnonymizeBEARAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bear.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBEARAmount:
            return QVariant(nAnonymizeBEARAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBEARAmount:
            nAnonymizeBEARAmount = value.toInt();
            settings.setValue("nAnonymizeBEARAmount", nAnonymizeBEARAmount);
            emit anonymizeBEARAmountChanged(nAnonymizeBEARAmount);
            break;
        case CoinControlFeatures:
