...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, 	// ONLY_NONDENOMINATED and not 1000 BEAR at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                      // UTXO's that are valid for staking
};

// Possible states for zBEAR send
enum ZerocoinSpendStatus {
    ZBEAR_SPEND_OKAY = 0,                            // No error
    ZBEAR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBEAR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBEAR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBEAR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBEAR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBEAR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBEAR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBEAR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBEAR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBEAR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBEAR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBEAR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBEAR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBEAR_SPENT_USED_ZBEAR = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZBEARAutoBackups(bool fEnabled)
    {
    ...
    /// Get 1000 BEAR output and keys which can be used for the Masternode
