...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            txNew.vout.push_back(CTxOut(Params().PublicFundReward(), CScript(DEV_SCRIPT.begin(), DEV_SCRIPT.end())));

            CAmount reductionFee = masternodePayment + Params().PublicFundReward();
            ...
            txNew.vout[0].nValue = blockValue - masternodePayment - Params().PublicFundReward();
        }
        ...
        if (fProofOfStake && pindexPrev->nHeight >= Params().PublicFundStart()) {
            unsigned int i = txNew.vout.size();
            ...
            txNew.vout[i - 1].nValue -= Params().PublicFundReward();
        }

        if (!fProofOfStake)
        {
            if (pindexPrev->nHeight >= Params().PublicFundStart()) {
                
                CTxDestination destination = CBitcoinAddress(Params().PublicFundAddress()).Get();
                CScript DEV_SCRIPT = GetScriptForDestination(destination);
                txNew.vout.push_back(CTxOut(Params().PublicFundReward(), CScript(DEV_SCRIPT.begin(), DEV_SCRIPT.end())));

                txNew.vout[0].nValue = blockValue - Params().PublicFundReward();
