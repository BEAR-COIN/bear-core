...
#if defined(NDEBUG)
#error "BEAR cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge2 = 12 * 60 * 60; // 12 * 60 * 60
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBEAR spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zBEAR spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zBEAR spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        nSubsidy = 9350860 * COIN;

/* 

INFO ABOUT PREMINE - Initial amount will be used for swap:
5 566 895 VoytekBEAR Coins + 3 733 965 CMTR Coins + 50 000 swap costs
After swap deadline (01-01-2021), unclaimed coins will go to masternode owners + Public Fund in scheme 50%-50%.
More details: https://bearcoin.net/go/discord (#swap text channel)

*/
    }
	else if (nHeight > 0 && nHeight <= 11649140) {
        nSubsidy = 1 * COIN;
    }
	else if (nHeight > 11649140) {
        nSubsidy = 0.00 * COIN; // 21 000 000 coins reached (after around 45 years)
    }
    else {
        nSubsidy = 1 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from BEAR
...
{
    RenameThread("bear-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBEARMinted()
{
...
void RecalculateZBEARSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBEAR supply
...
bool RecalculateBEARSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zBEAR with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBEARSpent: %s\n",
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    if (pindex->pprev->nHeight >= Params().PublicFundStart()) { 

      CTxDestination destination = CBitcoinAddress(Params().PublicFundAddress()).Get();
      CScript DEV_SCRIPT = GetScriptForDestination(destination);

      bool PublicFundPaid = false;
      for (const auto& output : block.vtx[(block.IsProofOfWork()?0:1)].vout) {
        if (output.scriptPubKey == DEV_SCRIPT && output.nValue == Params().PublicFundReward()) {
                PublicFundPaid = true;
                break;
            }
        }
        if (!PublicFundPaid) {
            return state.DoS(0, error("ConnectBlock(BEAR): no PublicFundPaid reward"), REJECT_INVALID, "PublicFund");
        }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BEAR to zBEAR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBEAR spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBEAR serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          // Inputs
          std::vector<CTxIn> bearInputs;
          std::vector<CTxIn> zBEARInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zBEARInputs.push_back(stakeIn);
              }else{
                  bearInputs.push_back(stakeIn);
              }
          }
          const bool hasBEARInputs = !bearInputs.empty();
          const bool hasZBEARInputs = !zBEARInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasBEARInputs)
                      ...
                      for (CTxIn bearIn : bearInputs){
                          if(bearIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zBEAR inputs.
              if(hasZBEARInputs){
                  for (CTxIn zBearInput : zBEARInputs) {
                      CoinSpend spend = TxInToZerocoinSpend(zBearInput);
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zbear");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBEAR mints and %d zBEAR spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BEAR: We use certain sporks during IBD, so check to see if they are
